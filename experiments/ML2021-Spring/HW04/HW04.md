...
| 参数 | 初始版 | V1版本 | 预计修改版（冲 strong baseline 0.95） |
|---|---:|---:|---:|
...
| warmup_steps | 1000 | 1000 | 2000 |
| total_steps | 70000 | 70000 | 60000 |
...
| 混合精度 | 无 | 无 | FP16 AMP (GradScaler + autocast) |

## 已修改说明

以下修改已全部写入 `HW04.ipynb`：

- 提升模型容量：`d_model=80 → 256`，Conformer 层数 `2 → 4`，`nhead=2 → 4`，`dim_feedforward=256 → 1024`。
- 降低过强正则：`dropout=0.3 → 0.15`。
- 改善说话人表征聚合：用 `AttentiveStatisticsPooling` 替代简单 mean pooling，拼接加权均值和标准差（输出维度 `d_model × 2 = 512`）。
- 分类头升级：`Linear(512→256) → BatchNorm1d → ReLU → Dropout → Linear(256→600)`。
- 训练加速：`batch_size=64 → 128`，`total_steps=70000 → 60000`（总数据量不变，步数减半缩短训练时间），`warmup_steps → 2000`。
- 混合精度训练：添加 FP16 AMP（`torch.amp.GradScaler` + `autocast`），在 T4 Tensor Core 上预期 2~3 倍加速，不影响最终精度。
  - **BF16 备选**：BF16 与 FP32 动态范围相同，无需 `GradScaler`，训练更稳定。但 BF16 Tensor Core 仅 Ampere 及以上架构（A100/RTX 30xx）支持，Colab T4（Turing 架构）无法使用，故采用 FP16。
